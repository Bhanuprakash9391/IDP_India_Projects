...
# Import LangGraph unified agent
from unified_agent_langgraph import UnifiedAgentLangGraph
...
class QnAAgentLangGraph:
    """
    Q&A Agent that can handle both document questions and task requests
    Uses LangGraph for task execution with better workflow management
    """

    def __init__(self):
        self.unified_agent = UnifiedAgentLangGraph()

    def classify_query_intent(self, query: str) -> Dict:
        """Classify whether the query is for information or task execution"""
        logger.info(f"LangGraph Q&A: Classifying query intent: {query}")
        try:
            client = AzureOpenAI(
            ...
            **MIXED QUERIES:**
            ...
            logger.info("LangGraph Q&A: Sending LLM request for intent classification")
            ...
            logger.info(f"LangGraph Q&A: LLM response received: {result_text}")

            # Extract JSON from response
            json_match = re.search(r'\{.*\}', result_text, re.DOTALL)
            if json_match:
                intent_result = json.loads(json_match.group())
                logger.info(f"LangGraph Q&A: Intent classification: {intent_result['intent']} (confidence: {intent_result['confidence']})")
                return intent_result
            else:
                logger.warning("LangGraph Q&A: Failed to parse JSON from LLM response, using fallback classification")
                return self._fallback_intent_classification(query)

        except Exception as e:
            logger.error(f"LangGraph Q&A: LLM intent classification error: {e}")
            return self._fallback_intent_classification(query)
        ...
        for doc_type, patterns in document_patterns.items():
            for pattern in patterns:
                ...
                if f"from {pattern}" in query_lower or f"in {pattern}" in query_lower or f"for {pattern}" in query_lower:
                    logger.info(f"LangGraph Q&A: Detected document type filter: {doc_type}")
                    return doc_type
    ...
    def filter_source_documents_by_type(self, source_docs: List, doc_type_filter: str, per_file_data: Dict) -> List:
        ...
        logger.info(f"LangGraph Q&A: Filtered {len(source_docs)} documents to {len(filtered_docs)} {doc_type_filter} documents")
        return filtered_docs

    def process_query(self, query: str, conversation_chain=None, per_file_data: Dict = None) -> Dict:
        """Process user query - either answer question or execute task using LangGraph"""
        logger.info(f"LangGraph Q&A: Processing query: {query}")
        ...
        if intent_result["intent"] == "TASK":
            logger.info("LangGraph Q&A: Query classified as TASK - delegating to LangGraph unified agent")
            # Use LangGraph unified agent for task execution
            if per_file_data:
            ...
            logger.info("LangGraph Q&A: Query classified as INFORMATION - using Q&A conversation")
            ...
            if conversation_chain:
                try:
                    ...
                    logger.error(f"LangGraph Q&A: Error in Q&A conversation: {e}")
                    return {
                        "type": "INFORMATION",
...
def integrate_qna_agent_langgraph_ui():
    """Integrate LangGraph Q&A Agent UI into the main application with continuous dialogue"""

    # Check if we have processed data
    if not st.session_state.get('per_file_data'):
        st.warning("Please process documents first to use the LangGraph Q&A Agent.")
        return

    st.markdown("### LangGraph Smart Q&A Agent")
    st.markdown("Ask questions about your documents or request actions - powered by LangGraph workflow management!")
    ...
    if "qna_conversation_langgraph" not in st.session_state:
        st.session_state.qna_conversation_langgraph = []

    # Display conversation history
    for message in st.session_state.qna_conversation_langgraph:
        role = message["role"]
    ...
    if user_query:
        ...
        st.session_state.qna_conversation_langgraph.append({"role": "user", "content": user_query})
        ...
        with st.chat_message("assistant"):
            with st.spinner("Analyzing your request with LangGraph..."):
                try:
                    qna_agent = QnAAgentLangGraph()
                    ...
                    if result["type"] == "INFORMATION":
                        ...
                        st.markdown("#### LangGraph Task Execution Details")
                        st.markdown(task_result["details"])

                        # Build response content for conversation history
                        response_content = f"**LangGraph Task Execution Result:**\n\n{task_result['summary']}\n\n{task_result['details']}"
                    ...
                    st.session_state.qna_conversation_langgraph.append({
                        "role": "assistant", 
