...
def integrate_excel_agent_ui():
    """Integrate Excel Agent UI into the main application"""
    
    # Check if we have processed data
    if not st.session_state.get('per_file_data'):
        st.warning("Please process documents first to use the Excel Agent.")
        return
    
    st.markdown("### Excel Agent")
    st.markdown("Create Excel files from your processed document data")
    
    # Query input
    query = st.text_area(
        "Enter your Excel creation query:",
        placeholder="Examples:\n- Create Excel with all document data\n- Create Excel with only filename and document type\n- Extract specific fields like document number and date\n- Create Excel for specific document types with selected fields",
        height=100
    )
    ...
    with col1:
        ...
        document_types = set()
        for file_data in st.session_state.per_file_data.values():
            doc_type = file_data.get('classification', {}).get('document_type', 'Unknown')
            if doc_type != 'Unknown':
        ...
        selected_doc_type = st.selectbox(
            ...
            ["All Documents"] + sorted(list(document_types))
        )
    
    with col2:
        # Output filename
        default_filename = f"document_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        output_filename = st.text_input("Output filename:", value=default_filename)
    
    # Create Excel from Query button - placed right below query input
    if st.button("Create Excel from Query", use_container_width=True, type="primary"):
        if query:
            with st.spinner("Creating Excel file..."):
                ...
                if selected_doc_type != "All Documents":
                    per_file_data = excel_agent.filter_documents_by_type(per_file_data, selected_doc_type)
                
                result = excel_agent.create_excel_from_query(query, per_file_data, output_path)
                st.success(result)
    ...
    # Comprehensive Excel button
    if st.button("Create Comprehensive Excel", use_container_width=True):
        with st.spinner("Creating comprehensive Excel file..."):
            excel_agent = ExcelAgent()
            output_path = os.path.join("exports", output_filename)
            
            # Apply document type filter if specified
            doc_type = None if selected_doc_type == "All Documents" else selected_doc_type
            
            result = excel_agent.create_comprehensive_excel(
            ...
            # Provide download link for single file (when specific document type is selected)
            if doc_type and os.path.exists(output_path):
                with open(output_path, "rb") as file:
                    st.download_button(
                        label="Download Excel File",
                        data=file,
