...
# Import Agents
from excel_agent import integrate_excel_agent_ui
from email_agent import integrate_email_agent_ui
from database_agent import integrate_database_agent_ui
...
def main():
    ...
    if st.session_state.per_file_data:
        # Create tab list: Q&A tab + Excel Agent + Email Agent + Database Agent + one tab per document
        tab_names = ["Q&A Interface", "Excel Agent", "Email Agent", "Database Agent"] + list(st.session_state.per_file_data.keys())
        ...
        # Tab 1: Excel Agent
        with tabs[1]:
            integrate_excel_agent_ui()
        
        # Tab 2: Email Agent
        with tabs[2]:
            integrate_email_agent_ui()
        
        # Tab 3: Database Agent
        with tabs[3]:
            integrate_database_agent_ui()
        
        # Tabs 4+: One tab per document
        for tab_idx, (file_name, file_data) in enumerate(st.session_state.per_file_data.items(), start=4):
            with tabs[tab_idx]:
