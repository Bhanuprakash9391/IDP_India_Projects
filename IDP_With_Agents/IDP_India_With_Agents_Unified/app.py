...
# Import Q&A Agent
from qna_agent import integrate_qna_agent_ui
...
def main():
    ...
    if st.session_state.per_file_data:
        ...
        # Welcome screen with available agents
        st.markdown("""
        ...
        st.markdown("Our intelligent agents can help you with various document processing tasks:")
        
        # Define agent information (concise one-line descriptions, no Unified Agent)
        agents_info = [
            {
                "name": "Q&A Agent",
            ...
            {
                "name": "Excel Agent", 
                "description": "Export document data to Excel spreadsheets"
            },
            {
                "name": "Email Agent",
                "description": "Send emails with extracted document information"
            },
            {
                "name": "Database Agent",
                "description": "Save structured data to PostgreSQL database"
            }
        ]
        ...
        for idx, agent in enumerate(agents_info):
            with cols[idx % 2]:
                st.markdown(f"""
                <div style="background: #F8F9FA; padding: 1rem; border-radius: 8px; border-left: 4px solid #6B2190; margin-bottom: 1rem;">
                    <h4 style="color: #6B2190; margin: 0 0 0.5rem 0;">🤖 {agent['name']}</h4>
                    <p style="margin: 0; color: #2c3e50; font-size: 0.9rem;">{agent['description']}</p>
                </div>
                """, unsafe_allow_html=True)
