...
def detect_and_extract_tables(page) -> List[Dict]:
        ...
        for block in blocks:
            if "lines" in block:
                ...
                if len(lines) > 2:
                    ...
                    if len(set(spans_per_line)) <= 2 and max(spans_per_line) > 1:
                        potential_table_blocks.append(block)
        
        for idx, table_block in enumerate(potential_table_blocks):
            table_text = []
            for line in table_block["lines"]:
                row_text = " | ".join([span["text"] for span in line.get("spans", [])])
...
def process_documents_with_intelligence(uploaded_files: list, client: AzureOpenAI):
    ...
    for file_idx, file in enumerate(uploaded_files):
        try:
            ...
            if file.type == "application/pdf":
                ...
                # STEP 4: Process each page for tables and document creation
                print(f"\n🔄 STEP 4: Processing tables and creating document embeddings...")
                ...
                for page_idx, page_data in enumerate(page_contents):
                    ...
                    # Detect tables
                    tables = detect_and_extract_tables(page)
                    if tables:
                        processing_stats["tables_found"] += len(tables)
                        print(f"    📊 Found {len(tables)} table(s) on page {page_num}")
                        
                        for table in tables:
                            structured_table = extract_table_with_llm_safe(client, table['content'])
                            ...
                            table_doc = Document(
                                page_content=json.dumps(structured_table, indent=2),
                                metadata={
                                    "source": file_name,
                                    "page": page_num,
                                    "type": "table",
                            ...
                            all_documents['table_documents'].append(table_doc)
    ...
    print(f"  Tables found: {processing_stats['tables_found']}")
...
def create_multiple_vectorstores(all_documents: Dict, db_name: str):
    ...
    if all_documents['table_documents']:
        st.write(f"Creating table vectorstore ({len(all_documents['table_documents'])} documents)...")
        vectorstores['table'] = FAISS.from_documents(all_documents['table_documents'], embeddings)
        vectorstores['table'].save_local(os.path.join(VECTOR_DB_DIR, f"{db_name}_table"))
...
def display_header():
    ...
    <div class="header-container">
        <h1 class="main-title">Intelligent Document Processing System</h1>
        <p class="subtitle">AI-Powered Classification, Extraction and Analysis</p>
    </div>
    """, unsafe_allow_html=True)
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.processing_stats:
            ...
            with col2:
                ...
                st.metric("Tables Found", stats.get('tables_found', 0))
    
    # Main content area - Create tabs
    if st.session_state.per_file_data:
        # Create tab list: Q&A tab + one tab per document
        tab_names = ["Q&A Interface"] + list(st.session_state.per_file_data.keys())
        ...
        for tab_idx, (file_name, file_data) in enumerate(st.session_state.per_file_data.items(), start=1):
            with tabs[tab_idx]:
                ...
                with col2:
                    ...
                    if tables:
                        with st.expander(f"Tables Detected ({len(tables)} tables)", expanded=True):
                            for table in tables:
                                st.markdown(f"**Table on Page {table.get('page', '?')}**")
                                ...
                                if 'error' not in structured:
                                    ...
                                    st.warning("Table structure extraction failed")
                                    st.text(table.get('raw_content', '')[:500])
                    ...
                    # Page-by-page info
                    pages_info = file_data.get('pages_info', [])
                    if pages_info:
                        with st.expander("Page-by-Page Processing Info"):
                            for page_info in pages_info:
                                page_num = page_info.get('page_num', '?')
                                method = page_info.get('processing_method', 'unknown')
                                tables_count = len(page_info.get('tables', []))
                                
                                st.markdown(f"**Page {page_num}:** Processed with {method}")
                                if tables_count > 0:
                                    st.markdown(f"  - {tables_count} table(s) found")
        ...
        <div style="background: #f8f9fa; padding: 2rem; border-radius: 10px; text-align: center;">
            <h2>Welcome to Intelligent Document Processing</h2>
            <p>Upload your documents to get started with:</p>
            <ul style="text-align: left; max-width: 600px; margin: 0 auto;">
                <li><strong>Automatic Classification</strong> - AI identifies document types</li>
                <li><strong>Structured Data Extraction</strong> - Intelligent field extraction</li>
                <li><strong>Table Detection & Extraction</strong> - Automatic table parsing</li>
                <li><strong>Multi-Database Architecture</strong> - Separate stores for text, tables, images</li>
                <li><strong>Smart Q&A</strong> - Query across all document types</li>
                <li><strong>Visual Caching</strong> - Efficient processing with result caching</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    if st.session_state.current_db_name:
        ...
        with col1:
            st.info(f"Database: {st.session_state.current_db_name}")
        with col2:
            num_dbs = len(st.session_state.vectorstores)
            st.info(f"Active Stores: {num_dbs}")
        with col3:
            num_docs = len(st.session_state.per_file_data)
