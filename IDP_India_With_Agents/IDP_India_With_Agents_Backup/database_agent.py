...
# Setup logging
def setup_logging():
    """Setup comprehensive logging for Database Agent"""
    log_dir = "logs"
    os.makedirs(log_dir, exist_ok=True)
    
    log_filename = f"{log_dir}/database_agent_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
class DatabaseAgent:
    ...
    def _filter_documents_for_database(self, per_file_data: Dict, filters: Dict) -> Dict:
        """Filter documents based on database filters"""
        if not filters:
...
def integrate_database_agent_ui():
    """Integrate Database Agent UI into the main application"""
    
    # Check if we have processed data
    if not st.session_state.get('per_file_data'):
        st.warning("Please process documents first to use the Database Agent.")
        return
    
    st.markdown("### 🗄️ Database Agent")
    st.markdown("Save extracted document data to PostgreSQL database")
    ...
    if not db_configured:
        st.warning("⚠️ Database configuration is incomplete. Please set DB_HOST, DB_USER, DB_PASSWORD, and DB_NAME in your .env file.")
    
    # Database query input
    db_query = st.text_area(
        "Enter your database save request:",
    ...
    if st.button("Save to Database", use_container_width=True, type="primary", disabled=not db_configured):
        if db_query:
            with st.spinner("Processing database request..."):
                db_agent = DatabaseAgent()
                result = db_agent.process_database_request(db_query, st.session_state.per_file_data)
            ...
            st.warning("Please enter a database request.")
    
    # Show existing tables
    if db_configured:
        st.markdown("---")
        st.markdown("#### Existing Database Tables")
                    ...
                    if tables:
                        ...
                        for table in tables:
                            with st.expander(f"📊 {table['table_name']} - {table['description']}"):
                                ...
                                # Show sample data
                                sample_data = db_agent.get_table_data(table['table_name'], 5)
                                if sample_data:
                        ...
                        st.info("No document tables found in the database.")
