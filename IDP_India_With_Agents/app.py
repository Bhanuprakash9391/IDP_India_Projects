...
def main():
    ...
    if st.session_state.per_file_data:
        # Create tab list: Q&A tab + Excel Agent + Email Agent + one tab per document
        tab_names = ["Q&A Interface", "Excel Agent", "Email Agent"] + list(st.session_state.per_file_data.keys())
        ...
        # Tabs 3+: One tab per document
        for tab_idx, (file_name, file_data) in enumerate(st.session_state.per_file_data.items(), start=3):
            with tabs[tab_idx]:
